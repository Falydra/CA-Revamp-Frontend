...
export default function DonateFundForm({
  campaign
}: DonateFundFormProps) {
        ...
        <div className="flex flex-col gap-4">
          ...
          <section className="flex flex-col gap-2">
            <label
              ...
              className="flex flex-col gap-2 text-gray-600">
              <span className="font-bold hover:cursor-pointer">Enter Donation Amount</span>
            </label>
            <div className="flex flex-row gap-2 h-12">
              <button
                ...
                onClick={(e) => setRequestData((prev) => ({ ...prev, fund_amount: 30000 }))}>
                Rp30.000
              </button>
              <button
                className="border w-full h-full items-center justify-center rounded-lg text-blue-600 font-medium border-blue-500 hover:bg-blue-500 hover:text-white active:bg-blue-600 transition-colors duration-100"
                onClick={(e) => setRequestData((prev) => ({ ...prev, fund_amount: 50000 }))}>
                Rp50.000
              </button>
              <button
                className="border w-full h-full items-center justify-center rounded-lg text-blue-600 font-medium border-blue-500 hover:bg-blue-500 hover:text-white active:bg-blue-600 transition-colors duration-100"
                onClick={(e) => setRequestData((prev) => ({ ...prev, fund_amount: 100000 }))}>
                Rp100.000
              </button>
            </div>
          </section>
          ...
          <section className="flex flex-col gap-2">
            <label
              htmlFor="donation-amount"
              className="flex flex-col gap-2 text-gray-600">
              <span className="font-bold hover:cursor-pointer">Or Enter Custom Amount</span>
              <div className="flex flex-row gap-1 border py-3 px-3 rounded-lg">
                ...
                <input
                  ...
                  id="donation-amount"
                  value={requestData.fund_amount ? formatPrice(requestData.fund_amount) : "0"}
                  onChange={(e) => {
              ...
              {requestData.fund_amount < 5000 && (
                <p className="text-red-500 text-sm">Donation below Rp5.000 is unavailable</p>
              )}
            </label>
          </section>
          ...
          <button
            className={`py-3 h-full rounded font-bold transition-colors duration-100
              ${requestData.fund_amount > 5000 ? "text-white bg-pink-500 cursor-pointer active:bg-pink-600" : "text-gray-600 bg-slate-300 cursor-not-allowed"}`}
